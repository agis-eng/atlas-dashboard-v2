...
export default function VoiceLabCleanPage() {
  ...
  return (
    <div className="p-6 md:p-8 xl:p-10 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight">Gemini Voice Lab Clean</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Fresh route. Raw Live API test only. No legacy voice picker behavior.
          </p>
        </div>
        {statusBadge}
      </div>
          ...
          <CardContent className="space-y-4">
            ...
            {sessionMeta && (
              <div className="rounded-md border border-border bg-muted/30 p-3 text-xs space-y-1 text-muted-foreground">
                <div>
                  <span className="font-medium text-foreground">Model:</span> {sessionMeta.model}
                </div>
                {sessionMeta.expiresAt && (
                  <div>
                    <span className="font-medium text-foreground">Token expires:</span> {new Date(sessionMeta.expiresAt).toLocaleTimeString()}
                  </div>
                )}
              </div>
            )}
