...
Return ONLY the unsubscribe URL, nothing else. If no unsubscribe link found, return "NOT_FOUND".`,
    ...
    // For now, just return the URL for manual unsubscribe
    // TODO: Integrate with OpenClaw browser tool for automated clicking
    return NextResponse.json({
      success: true,
      message: `Found unsubscribe link! Opening in new tab. Click the unsubscribe button to complete.`,
      url: unsubscribeUrl,
      manualAction: true,
    });
  } catch (error: any) {
